...
//      ticket->SetResponseText(args);
        if(m_session && m_session->GetPlayer())
            ticket->SetResponseText(args, m_session->GetPlayer()->GetGUIDLow());
        else
            ticket->SetResponseText(args, sConfig.GetIntDefault("TicketWebGmID", 3));
//
...
bool ChatHandler::HandleMmapPathCommand(char* args)
{
    ...
    PSendSysMessage("length %i type %u", (int)(pointPath.size()), path.getPathType());
